...
sum_Si=zeros(11,n_grid,n_grid)
sum_Sj=zeros(11,n_grid,n_grid)
sum_SiSj=zeros(11,n_grid,n_grid)
...
#################Energy change after spin flip######
        ...
        ########## Correlation Calculation ##############
            p=1
            for i=convert(Int,floor(n_grid/2))-5:1:convert(Int,floor(n_grid/2))+4
              for f in 1:n_grid  
                for r in 1:n_grid-2
                    Si = grid[i,f]
                    Sj = grid[i,mod(r+f,n_grid)]
                    sum_Si[p,f,mod(r+f,n_grid)]+=Si
                    sum_Sj[p,f,mod(r+f,n_grid)]+=Sj
                    sum_SiSj[p,f,mod(r+f,n_grid)]+=Si*Sj
                end
              end
                p+=1
            end
